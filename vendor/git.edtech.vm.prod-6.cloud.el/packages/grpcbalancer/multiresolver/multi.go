 ...
 * ```
 * import _ "git.edtech.vm.prod-6.cloud.el/packages/grpcbalancer/multiresolver"
 * ```
 */
...
func (builder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	for _, t := range strings.Split(target.URL.Path, ",") {
		...
		res, err := resolverBuilder.Build(parsedTarget, pcc, opts)
		if err != nil {
			mr.Close()
			return nil, err
		}
		mr.children = append(mr.children, res)
	}
	return mr, nil
}
