...
type RenderCtx struct {
	...
	Inc     bool                   `json:"inc"`
	ACL     string                 `json:"acl" lcml:"acl"` // зашифрованный токен
	...
	// поля для RDS-сервера
	Command string `json:"command" lcml:"command"`
	Status  string `json:"status" lcml:"status"`
	Hash    string `json:"hash" lcml:"hash"`
...
func ByteToRenderCtx(body []byte) (res RenderCtx, err error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "//") && !flagParam {
			...
			if len(value) != 0 && len(splitLine) == 2 {
