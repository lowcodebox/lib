...
// на стороне AuthProcessor приложения, а также данные новой роли в хранилище сессии, поскольку токен будет завершенны
...
func (s *service) AuthChangeRole(ctx context.Context, in model.ServiceAuthIn) (out model.ServiceAuthOut, err error){
	status, _, refreshToken, err := s.iam.Verify(fmt.Sprint(ctx.Value("token")))
	if err != nil {
	...
	if status {
		...
		out.Token, err = s.iam.Refresh(refreshToken, in.Profile, in.Expire)
		if err != nil {
